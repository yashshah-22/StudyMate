...
//       toast.success('Congratulations! You have cleared the Final test !!! Well Done!!', {
//         position: 'top-right',
//         autoClose: 3000,
...
//         draggable: true,
//       });
//       navigate(`/test/${subject}/finalcertificate`);

//     } else {
//       toast.error('You did not clear the test! Retake the final test again.', {
//         position: 'top-right',
//         autoClose: 3000,
//         hideProgressBar: false,
//         closeOnClick: true,
...
import axios from "axios";
import { useNavigate } from 'react-router-dom';
import Certificate from './Certificate'; // Replace with your actual Certificate component
...
const FinalTest = () => {
  ...
  const handleSubmitAnswers = async () => {
    ...
    if (currentScore === questions.length) {
      ...
      setTimeout(() => {
        setLoading(false);
        // Redirect or display certificate here
        // For demonstration purposes, I'm assuming you have a Certificate component.
        // Replace this with your actual certificate display logic.
        navigate(`/test/${subject}/finalcertificate`);
      }, 4000);
    } else {
      toast.error('You did not clear the test! Retake the final test again.', {
    ...
    <>
      {loading ? (
        <div className="loading-message">
          <p>Generating Certificate. Please wait...</p>
        </div>
      ) : (
        <div className="test-page">
          <div className="timer" style={{ fontSize: "30px", position: "static", "marginLeft": "1000px", "marginTop": "35px" }}>
            Time Left: {Math.floor(remainingTime / 60)}:{(remainingTime % 60).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}
          </div>
          <h1>Final {subject} Test</h1>
          <hr className="divider" />
          <ul className="question-list">
